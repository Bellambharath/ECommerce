...
export class CartComponent implements OnInit, AfterViewInit {
  ...
  CheckOut() {
    ...
    for (var i = 0; i < this.SelectArray.length; i++) 
    {
      ...
      {
        insidecount++;
        let product = this.p.filter(x => x.productId == this.UserBasedCart[i].productId)
        let productqty = product[0].noofstocks

        if (this.QuentityCount[i] <= productqty) 
        {
          count++;
        }
      }
  ...
  openSnackBar() 
  {
   let message="Products Unavailable,\nsome products in your bag are out of stock";
    let action="ok";
